...
class ScannerService {
  ...
  List<Candle> buildCandles(List<TradePoint> trades, Duration timeframe) {
    ...
    for (final trade in sorted) {
      ...
      if (current == null) {
        map[bucket] = Candle(timestampMs: bucket, open: trade.price, high: trade.price, low: trade.price, close: trade.price, volume: trade.quantity);
      } else {
        map[bucket] = Candle(timestampMs: bucket, open: current.open, high: trade.price > current.high ? trade.price : current.high, low: trade.price < current.low ? trade.price : current.low, close: trade.price, volume: current.volume + trade.quantity);
      }
    }
  ...
  double _ema(List<double> values, int period) {
    ...
    for (final value in values.skip(period)) ema = (value - ema) * multiplier + ema;
    return ema;
  }
  ...
  double _atr(List<Candle> candles, int period) {
    ...
    for (var i = 1; i < candles.length; i++) {
      final c = candles[i], prev = candles[i - 1];
      trs.add([c.high - c.low, (c.high - prev.close).abs(), (c.low - prev.close).abs()].reduce((a, b) => a > b ? a : b));
    }
  ...
  Future<MarketSignal?> scanSymbol(String symbol, Duration timeframe) async {
    ...
    final ema9 = _ema(closes, 9), ema21 = _ema(closes, 21);
    ...
    for (final row in (depth['bids'] is List ? depth['bids'] : const [])) {
      if (row is List && row.length >= 2) bid += double.tryParse('${row[1]}') ?? 0;
    }
    for (final row in (depth['asks'] is List ? depth['asks'] : const [])) {
      if (row is List && row.length >= 2) ask += double.tryParse('${row[1]}') ?? 0;
    }
  ...
  Future<List<MarketSignal>> scanAll({Duration timeframe = const Duration(minutes: 15), int maxConcurrency = 4}) async {
    ...
    for (var start = 0; start < symbols.length; start += maxConcurrency) {
      ...
      final results = await Future.wait(batch.map((s) async { try { return await scanSymbol(s, timeframe); } catch (_) { return null; } }));

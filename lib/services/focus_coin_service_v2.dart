...
class FocusCoinServiceV2 {
  ...
  Future<FocusSnapshotV2> tick({
    ...
    final longs = scanned.where((s) => s.side.toUpperCase() == 'LONG').toList()
    ...
    if (_focusSymbol != null) {
      pool.addAll(longs.where((s) => s.symbol == _focusSymbol));
    }
    ...
    for (final signal in pool) {
      try {
        ...
        final usable = !r.dataStale && !r.dataInsufficient &&
            r.decision == CoinDecision.buy && r.hasTradePlan &&
            (r.riskReward ?? 0) >= 1.4;
        if (usable) {
          evaluated.add(_FocusCandidate(signal, r, r.score * 0.65 + r.confidence * 0.35));
        }
      } catch (_) {}
    }
    ...
    if (_focusSymbol != null) {
      for (final c in evaluated) {
        if (c.signal.symbol == _focusSymbol) { current = c; break; }
      }
    }
    ...
    final start = chosen.result.score >= startScore && chosen.result.confidence >= 65;
    final action = switched ? FocusActionV2.switchFocus : (start ? FocusActionV2.startNow : FocusActionV2.wait);
    ...
    return FocusSnapshotV2(
      symbol: chosen.signal.symbol, action: action, score: chosen.result.score,
      confidence: chosen.result.confidence, side: 'LONG',
      ...
      timeframe: chosen.signal.timeframe, reasonsFa: reasons, at: now,
      dataSource: chosen.result.dataSource, signal: chosen.signal,
    );
  }
}
...
class FocusSnapshotV2 {
  const FocusSnapshotV2({required this.symbol, required this.action, required this.score, required this.confidence, required this.side, required this.entry, required this.stopLoss, required this.tp1, required this.tp2, required this.tp3, required this.riskReward, required this.timeframe, required this.reasonsFa, required this.at, required this.dataSource, required this.signal});
  final String? symbol;
  ...
  factory FocusSnapshotV2.empty(DateTime now, String source) => FocusSnapshotV2(
    symbol: null, action: FocusActionV2.noSetup, score: 0, confidence: 0, side: 'NONE',
    entry: null, stopLoss: null, tp1: null, tp2: null, tp3: null, riskReward: 0,
    timeframe: '15m', reasonsFa: const ['هیچ موقعیت LONG پس از تحلیل عمیق چندتایم‌فریم تأیید نشد', 'امتیاز مصنوعی اضافه نشد؛ فعلاً انتظار'],
    at: now, dataSource: source, signal: null,
  );
}

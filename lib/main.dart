              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    const CircleAvatar(
                      radius: 38,
                      child: Icon(Icons.candlestick_chart, size: 38),
                    ),
                    ...
                    TextField(
                      ...
                      obscureText: true,
                      decoration: InputDecoration(
                        labelText: en ? 'Password' : 'رمز عبور',
                    ...
                    SizedBox(
                      ...
                      child: FilledButton.icon(
                        ...
                        label: Text(en ? 'Sign in' : 'ورود'),
                      ),
                    ),
                    const SizedBox(height: 12),
                    Text(
                      en ? 'Only the owner account is accepted.' : 'فقط حساب مالک برنامه پذیرفته می‌شود.',
...
class _HomePageState extends State<HomePage> {
  ...
  Future<List<dynamic>> trades(String symbol) async {
    final uri = Uri.parse('$apiBase/r/api/v1/trades').replace(
      queryParameters: {'symbol': symbol, 'limit': '300'},
    );
  ...
  Future<Map<String, dynamic>> depth(String symbol) async {
    final uri = Uri.parse('$apiBase/r/api/v1/depth').replace(
  ...
  Future<void> scan() async {
    ...
    for (final symbol in symbols) {
      try {
        ...
        // One unavailable market must not stop the complete scan.
      }
    }
    ...
    setState(() {
      ...
      status = candidates.isEmpty
          ? (widget.english ? 'No valid setup found.' : 'سیگنال معتبر پیدا نشد.')
          : (widget.english ? 'Market scan completed.' : 'اسکن بازار انجام شد.');
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(en ? 'Signal Scanner' : 'اسکنر سیگنال'),
        actions: [
          IconButton(
        ...
        child: ListView(
          ...
          children: [
                      ...
                      children: [
                        Expanded(
                          child: DropdownButtonFormField<String>(
                            ...
                            decoration: InputDecoration(labelText: en ? 'Timeframe' : 'تایم‌فریم'),
                            items: ['1m', '5m', '15m', '1h']
                        ...
                        Expanded(
                          child: TextFormField(
                            ...
                            decoration: InputDecoration(labelText: en ? 'Capital (Toman)' : 'سرمایه (تومان)'),
                            onChanged: (value) => capital = double.tryParse(value.replaceAll(',', '')) ?? capital,
                          ),
                        ),
                      ],
                    ),
                    ...
                    SizedBox(
                      ...
                      child: FilledButton.icon(
                        ...
                        label: Text(loading
                            ...
                            : (en ? 'Find best opportunity' : 'پیدا کردن بهترین فرصت')),
                      ),
                    ),
                  ],
                ),
            ...
            if (status != null && !loading)
              Card(child: ListTile(leading: Icon(signal == null ? Icons.block : Icons.check_circle), title: Text(status!))),
            ...
            Card(
              child: ListTile(
                ...
                subtitle: Text(en ? 'No orders are sent to the exchange.' : 'هیچ سفارشی به صرافی ارسال نمی‌شود.'),
              ),
            ),
          ],
        ),
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Text(signal.symbol, style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)),
                Chip(label: Text(signal.side)),
              ],
            ),
            Text('${en ? 'Timeframe' : 'تایم‌فریم'}: ${signal.timeframe}'),
            const Divider(height: 28),
            _row(context, en ? 'Signal score' : 'امتیاز سیگنال', '${signal.score.toStringAsFixed(0)}/100'),
            _row(context, en ? 'Entry' : 'نقطه ورود', number(signal.entry)),
            _row(context, en ? 'Stop Loss' : 'حد ضرر', number(signal.stop)),
            _row(context, en ? 'Take Profit' : 'حد سود', number(signal.takeProfit)),
            _row(context, en ? 'Risk/Reward' : 'ریسک به سود', '1 : 2.5'),
          ],
        ),
      ),
    );

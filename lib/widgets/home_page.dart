  ...
  void initState() {
    ...
    androidNotify = AndroidNotificationService(
      onSelect: (payload) {
        ...
        final sym = parsed.item1;
        final side = parsed.item2;
        MarketSignal? match;
        for (final s in signals) {
          if (s.symbol.toUpperCase() == sym && s.side.toUpperCase() == side) {
            match = s;
            break;
          }
        }
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              widget.english
                  ? 'Opportunity: ${hit.symbol} ${hit.side}'
                  : 'فرصت: ${hit.symbol} ${hit.side}',
            ),
            action: SnackBarAction(
              label: widget.english ? 'Open' : 'باز کردن',
              onPressed: () => placeOnPhone(hit, isOpen: true),
            ),
          ),
        );
      },
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (realtime?.lastSuccess != null)
                          Text(
                            en
                                ? 'Last: ${realtime!.lastSuccess}'
                                : 'آخرین: ${realtime!.lastSuccess}',
                            style: Theme.of(context).textTheme.bodySmall,
                          ),
                        if (realtime != null)
                          Text(
                            en
                                ? 'Scan budget: ${realtime!.maxSymbols} symbols'
                                : 'سقف اسکن: ${realtime!.maxSymbols} نماد',
                            style: Theme.of(context).textTheme.bodySmall,
                          ),
